...
export default function Navbar() {
    ...
    return (
        <>
            {/* ─── Announcement Bar ─── */}
            <div className="bg-[#0c5c2b] text-white text-xs py-2 text-center overflow-hidden select-none">
                <span
                ...
                >
                    ...
                    <Link href="/shop" className="ml-3 text-[#FBB21B] font-bold underline underline-offset-2">Shop →</Link>
                </span>
            </div>
            ...
            <header className="sticky top-0 z-50 bg-white border-b border-gray-100">
                ...
                <div className="hidden md:flex items-stretch max-w-screen-xl mx-auto" style={{ minHeight: '72px' }}>
                        ...
                        {LEFT_LINKS.map(link => (
                            <Link
                                ...
                                className={`relative px-4 py-2 text-sm font-semibold tracking-wide rounded-lg transition-colors
                                    ${isActive(link.href)
                                        ? 'text-[#0c5c2b]'
                                        : 'text-gray-600 hover:text-[#0c5c2b] hover:bg-green-50'
                                    }`}
                            >
                                {link.label}
                                {isActive(link.href) && (
                                    <span className="absolute bottom-0 left-1/2 -translate-x-1/2 w-4 h-0.5 bg-[#FBB21B] rounded-full" />
                                )}
                            </Link>
                        ))}
                    </nav>
                    ...
                    <div className="flex flex-1 items-center justify-end gap-1 px-6">
                        ...
                        >
                            <i className={`text-lg ${isAuthenticated ? 'fa-solid fa-user text-[#0c5c2b]' : 'fa-regular fa-user text-gray-500 group-hover:text-[#0c5c2b]'}`} />
                            <span className="text-[10px] font-semibold text-gray-500 group-hover:text-[#0c5c2b] uppercase tracking-wide">
                        ...
                        <Link href="/profile?tab=wishlist" className="flex flex-col items-center gap-0.5 px-3 py-2 rounded-lg hover:bg-gray-50 transition-colors group">
                            <i className="fa-regular fa-heart text-lg text-gray-500 group-hover:text-[#0c5c2b]" />
                            <span className="text-[10px] font-semibold text-gray-500 group-hover:text-[#0c5c2b] uppercase tracking-wide">Wishlist</span>
                        </Link>

                        {/* Cart */}
                        <Link href="/cart" className="flex flex-col items-center gap-0.5 px-2 py-2 rounded-lg group">
                            <div className="relative bg-[#FBB21B] group-hover:bg-yellow-400 text-[#2c1a0e] w-10 h-10 flex items-center justify-center rounded-xl shadow-sm group-hover:shadow-md transition-all">
                                <i className="fa-solid fa-cart-shopping text-base" />
                                {cartQty > 0 && (
                                    <span className="absolute -top-1.5 -right-1.5 bg-[#0c5c2b] text-white text-[9px] font-black w-[18px] h-[18px] flex items-center justify-center rounded-full shadow">
                                        {cartQty > 9 ? '9+' : cartQty}
                                    </span>
                                )}
                            </div>
                            <span className="text-[10px] font-semibold text-gray-500 group-hover:text-[#0c5c2b] uppercase tracking-wide">Cart</span>
                        </Link>
                    </div>
                </div>
                ...
                <div className="md:hidden flex items-center justify-between px-4 py-3">
                    ...
                    <Link href="/cart" className="relative w-10 h-10 flex items-center justify-center rounded-xl bg-[#FBB21B] hover:bg-yellow-400 transition-colors">
                        <i className="fa-solid fa-cart-shopping text-[#2c1a0e] text-lg" />
                        {cartQty > 0 && (
                            <span className="absolute -top-1 -right-1 bg-[#0c5c2b] text-white text-[9px] font-black w-4 h-4 flex items-center justify-center rounded-full">{cartQty}</span>
                        )}
                    </Link>
                </div>
            </header>
                                ...
                                <Link key={item.href} href={item.href}
                                    className={`flex items-center gap-3 px-4 py-3.5 rounded-xl font-semibold text-sm transition-colors
                                        ${isActive(item.href) && item.href !== '/'
                                            ? 'bg-green-50 text-[#0c5c2b]'
                                            : 'text-gray-700 hover:bg-gray-50 hover:text-[#0c5c2b]'}`
                                    }
                                >
                                    <i className={`fa-solid ${item.icon} w-4 text-[#0c5c2b] text-sm`} />
                                    {item.label}
                                </Link>
                            ))}
                                ...
                                {isAuthenticated ? (
                                    <>
                                        <Link href="/profile" className="flex items-center gap-3 px-4 py-3.5 rounded-xl text-gray-700 hover:bg-gray-50 font-semibold text-sm">
                                            <i className="fa-solid fa-user w-4 text-[#0c5c2b] text-sm" /> My Profile
                                        </Link>
                                    ...
                                    <>
                                        <Link href="/login" className="flex items-center justify-center gap-2 px-4 py-3.5 rounded-xl bg-[#0c5c2b] text-white font-bold text-sm">
                                            <i className="fa-solid fa-right-to-bracket" /> Sign In
                                        </Link>
                                        <Link href="/register" className="flex items-center justify-center gap-2 px-4 py-3.5 rounded-xl border-2 border-[#0c5c2b] text-[#0c5c2b] font-bold text-sm">
