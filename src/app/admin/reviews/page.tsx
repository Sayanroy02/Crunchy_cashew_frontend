...
function StarDisplay({ rating = 5 }: { rating?: number }) {
    return (
        <div className="flex gap-0.5">
            {[1, 2, 3, 4, 5].map(s => (
                <i key={s} className={`text-sm ${s <= rating ? 'fa-solid fa-star text-amber' : 'fa-regular fa-star text-gray-200'}`} />
            ))}
        </div>
    );
}
    ...
    return (
        <div className="flex flex-col gap-6">
            <div className="flex items-center justify-between flex-wrap gap-3">
                ...
                <button onClick={fetchTestimonials} className="text-sm text-primary font-semibold flex items-center gap-1.5 hover:underline">
                        ...
                        <div key={t._id} className={`bg-white rounded-2xl border p-5 shadow-sm transition-all ${t.approved ? 'border-green-100' : 'border-orange-100'}`}>
                            <div className="flex items-start justify-between mb-3">
                                <div className="flex items-center gap-3">
                                    <div className="w-9 h-9 rounded-xl bg-primary text-white flex items-center justify-center font-black text-base">
                            ...
                            <div className="flex gap-2 mt-4 pt-3 border-t border-gray-50">
                                {!t.approved && (
                                    <button onClick={() => handleApprove(t._id)}
                                        className="flex-1 bg-primary text-white text-xs font-bold py-2 rounded-xl hover:bg-green-800 transition flex items-center justify-center gap-1.5">
